...
public class Group extends AppCompatActivity {
    ...
    private void getdata(){
        ...
        while(cs.moveToNext()){
            if(GameName.equals(cs.getString(cs.getColumnIndex("name")))){
                group=cs.getInt(cs.getColumnIndex("groupNum"));
                groupPlayerNum=cs.getInt(cs.getColumnIndex("groupPlayerNum"));
            }
        }
    ...
    private void inputTitleDialog(final int n) {
        ...
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                ...
                //数据库中读取比赛的小组人数
                Cursor cs2 = myDataBase.select("select groupPlayerNum from game where name='" + GameName + "'");
                cs2.moveToNext();
                //人数判断
                if (playerNum >= cs2.getInt(cs2.getColumnIndex("groupPlayerNum"))) {
